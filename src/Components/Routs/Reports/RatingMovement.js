...
function RatingMovement(props) {
  const [RatingMovement, setRatingMovement] = useState([]);
  ...
  const getTabsData = async (fromDate, toDate) => {
    const res = GetGridsApi.RatingMovementApi(fromDate, toDate);
    const resp = await res;
    setRatingMovement(resp.data);
  };
  ...
  const PvtRatingsdatefilters = (
    ...
    var dateFilterComponent = gridApi.api.getFilterInstance("notification_date");
