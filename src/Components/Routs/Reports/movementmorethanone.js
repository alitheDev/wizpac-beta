...
export default function Movementmorethanone(props) {
    const [Movementmorethanone, setMovementmorethanone] = useState([]);
    ...
    const getTabsData = async (fromDate, toDate) => {
      const res = GetGridsApi.Movementmorethanoneform(fromDate, toDate);
      const resp = await res;
      console.log(resp.data,'ressspppp');
      setMovementmorethanone(resp.data);
    };
    ...
    const columnDefs = [
      ...
      {
        headerName: "Difference in Notches",
        field: "notch_diff",
        minWidth: 130,
        minWidth: 130,
        sortable: true,
    ...
    return (
      <>
        <TableComponent
          value={"Movementmorethanone"}
          handleRfreshTab={props.handleRfreshTab}
          Data={Movementmorethanone}
