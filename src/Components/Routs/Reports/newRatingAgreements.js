...
export default function NewRatingAgreements(props) {
    const [newRatingAgreements, setNewRatingAgreements] = useState([]);
    ...
    const getTabsData = async (fromDate, toDate) => {
      const res = GetGridsApi.NewRatingAgreementsApi(fromDate, toDate);
      const resp = await res;
      console.log(resp.data,'ressspppp');
      setNewRatingAgreements(resp.data);
    };
    ...
    const columnDefs = [
      ...
      {
        headerName: "Total Asset Base / Issue size(PKR mln)",
        minWidth: responsiveColumns(),
        field: "asset_size",
        sortable: true,
        filter: "agSetColumnFilter",
    ...
    return (
      <>
        <TableComponent
          value={"newRatingAgreements"}
          handleRfreshTab={props.handleRfreshTab}
          Data={newRatingAgreements}
