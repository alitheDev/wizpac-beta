...
export default function SwitchRating(props) {
    ...
    const getTabsData = async (fromDate, toDate) => {
      const res = GetGridsApi.RatingSwitchApi(fromDate, toDate);
      const array1 = [];
      const resp = await res;
      const arr = Object.keys(resp.data.initials);
        const arr2 = Object.keys(resp.data.secondLast);
        console.log(arr.length, arr2.length);
        const array2 = [];

        for (let i in arr) {
          array1.push(resp.data.initials[arr[i]][0]);
        }

        for (let i in arr2) {
          array2.push(resp.data.secondLast[arr2[i]][0]);
        }

        array1.sort((a, b) => {
          const nameA = a.name.toUpperCase(); // convert to uppercase to ignore casing
          const nameB = b.name.toUpperCase(); // convert to uppercase to ignore casing
        ...
        for (let i in array1) {
            if(array1[i].name == array2[i].name){
                array1[i].dissemination_date = array2[i].dissemination_date;
                array1[i].title1 = array2[i].title;
            }
        }

        setswitchRating(array1)
    };
    ...
    const columnDefs = [
      ...
      {
        headerName: "Rating Assigned(PACRA)",
        minWidth: responsiveColumns(),
        field: "title1",
        sortable: true,
        filter: "agSetColumnFilter",
      ...
      {
        headerName: "Date of Rating Assigned (PACRA withdrawal date)",
      ...
      {
        headerName: "Switch rating if any(VIS Initial Rating)",
        minWidth: responsiveColumns(),
        field: "title",
        sortable: true,
        filter: "agSetColumnFilter",
      ...
      {
        headerName: "Date of switch rating(JCR)",
        field: "jcr_dissemination_date",
        minWidth: 115,
        // hide: true,
        sortable: true,
        filter: "agDateColumnFilter",
