...
function DeadlineIc(props) {
  ...
  function last_diss(params) {
    if (params.value === "nullData") return <Skeleton />;
    if (params.data.last_diss == null) {
      return "-";
    } else {
      const date = new Date(params.data.last_diss);
      const yyyy = date.getFullYear();
      const yy = yyyy.toString();
      const y = yy.slice(2, 4);
      let mm = date.toLocaleString("default", { month: "short" });
    ...
    <h1 style={{ padding: "10px 20px" }}>
      <Table className="overflow-scroll responsiveTableFonts">
        ...
        <TableRow>
          ...
          <TableCell className="responsiveTableFonts py-1">
            {params.data.days_to_ic === "" ||
            params.data.days_to_ic == null ||
            params.data.days_to_ic == 100000 ? (
              "-"
            ) : params.data.days_to_ic < 0 ? (
              <span className="text-danger"> {params.data.days_to_ic} </span>
            ) : (
              <span className="text-success"> {params.data.days_to_ic} </span>
            )}
          </TableCell>
        </TableRow>
      </Table>
  ...
  return (
    <TableComponent
      value={"DeadlineIc"}
      Data={props.DeadlineIc}
      columnDefs={columnDefs}
      screenWidth={props.screenWidth}
      MobViewRender={MobViewRender}
      mainTabValue={props.mainTabValue}
      datefilters={DeadlineIcdatefilters}
      datefilter={true}
    />
  );
}

export default DeadlineIc;
