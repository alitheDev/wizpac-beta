...
import { Event, MailOutline } from "@material-ui/icons";
import { Skeleton, Table, TableCell, TableRow } from "@mui/material";

function InRIP(props) {
  ...
  const inRIPfilters = (startDate, endDate, gridApi, getFilterType) => {
    ...
    },
    {
      ...
      cellRenderer: function (params) {
        if (params.value === "nullData") return <Skeleton />;
          ...
          return (
            <a
              href={`https://209.97.168.200/pacrawizpackv3/public/admin/viewnl-new/${params.data.Id}/${params.data.recordid}`}
              target="_blank"
              rel="noopener noreferrer"
            >
              <MailOutline className="theme_text" />
            </a>
          );
      },
      excelMode: "windows",
