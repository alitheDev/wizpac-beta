...
function DoneFc(props) {
  ...
  const DoneFcdatefilters = (startDate, endDate, gridApi, getFilterType) => {
  ...
  function fc_date_current(params) {
    if (params.data.fc_date_current == null) {
      return "-";
    } else {
      const date = new Date(params.data.fc_date_current);
      const yyyy = date.getFullYear();
      const yy = yyyy.toString();
      const y = yy.slice(2, 4);
      let mm = date.toLocaleString("default", { month: "short" });
  ...
  return (
    <TableComponent
      ...
      datefilters={DoneFcdatefilters}
      datefilter={true}
    />
  );
}

export default DoneFc;
